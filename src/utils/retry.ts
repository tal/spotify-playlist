...
const DEFAULT_CONFIG: Required<RetryConfig> = {
  ...
  onRetry: (error, attempt, nextDelay) => {
    console.log(`⚠️ Retry attempt ${attempt} after ${nextDelay}ms due to:`, error.message || error)
  }
}
...
export async function retryWithBackoff<T>(
  ...
  for (let attempt = 1; attempt <= finalConfig.maxRetries; attempt++) {
    try {
      ...
      let nextDelay = finalConfig.initialDelay * Math.pow(finalConfig.backoffMultiplier, attempt - 1)
...
export async function retrySpotifyCall<T>(
  ...
  return retryWithBackoff(operation, {
    ...
    onRetry: (error, attempt, nextDelay) => {
      ...
      if (isTimeout) {
        console.log(`⏱️ ${operationName} timed out, retrying attempt ${attempt} after ${nextDelay}ms`)
      } else if (isRateLimit) {
        console.log(`🚦 ${operationName} rate limited, retrying attempt ${attempt} after ${nextDelay}ms`)
      } else {
        console.log(`⚠️ ${operationName} failed, retrying attempt ${attempt} after ${nextDelay}ms`)
      }
    }
  })
}