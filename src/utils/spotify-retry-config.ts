...
export function getSpotifyRetryConfig(): SpotifyRetryOptions {
  ...
  if (process.env.SPOTIFY_RETRY_BACKOFF_MULTIPLIER) {
    const backoffMultiplier = parseFloat(process.env.SPOTIFY_RETRY_BACKOFF_MULTIPLIER)
    if (!isNaN(backoffMultiplier)) {
      config.default!.backoffMultiplier = backoffMultiplier
      config.savedTracks!.backoffMultiplier = Math.min(backoffMultiplier * 1.25, 3) // Slightly more aggressive for saved tracks
    }
  }
